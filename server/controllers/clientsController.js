...
class ClientsController {
    ...
    async getAll(req, res) {
        ...
        let clients
        if(name.length <= 2){
            clients = await Client.findAndCountAll({limit, offset, order: [['surname', 'ASC']]})
        } else {
            clients = await Client.findAndCountAll({limit, offset, where: { 'surname': { [Op.like]: '%' + name.charAt(0).toUpperCase() + name.toLowerCase().slice(1) + '%' }}, order: [['surname', 'ASC']]})
        }
